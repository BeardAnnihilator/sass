...
class SuperselectorTest < Test::Unit::TestCase
  ...
  def test_nth_match_is_superselector_of_subset_nth_match
    assert_strict_superselector(
      ':nth-column(2n of .foo, .bar, .baz)', '#x:nth-column(2n of .foo.bip, .baz.bang)')
    assert_strict_superselector(
      ':nth-last-column(2n of .foo, .bar, .baz)', '#x:nth-last-column(2n of .foo.bip, .baz.bang)')
  end

  def test_nth_match_is_not_superselector_of_nth_match_with_different_arg
    assert_not_superselector(
      ':nth-column(2n of .foo, .bar, .baz)', '#x:nth-column(2n + 1 of .foo.bip, .baz.bang)')
    assert_not_superselector(
      ':nth-last-column(2n of .foo, .bar, .baz)', '#x:nth-last-column(2n + 1 of .foo.bip, .baz.bang)')
  end

  def test_nth_match_is_not_superselector_of_nth_last_match
    assert_not_superselector ':nth-column(2n of .foo, .bar)', ':nth-last-column(2n of .foo, .bar)'
    assert_not_superselector ':nth-last-column(2n of .foo, .bar)', ':nth-column(2n of .foo, .bar)'
  end

  def test_nth_match_can_be_subselector
    %w[nth-column nth-last-column].each do |name|
      assert_not_superselector '.foo', ":#{name}(2n of .foo.bar)"
      assert_not_superselector '.foo.bar', ":#{name}(2n of .foo.bar.baz)"
      assert_not_superselector '.foo', ":#{name}(2n of .foo.bar, .foo.baz)"
    end
  end
