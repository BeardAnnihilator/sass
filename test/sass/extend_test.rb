...
SCSS
  ...
  def test_extend_into_not
    assert_unification(':not(.foo)', '.x {@extend .foo}', ':not(.foo):not(.x)')
    assert_unification(':not(.foo.bar)', '.x {@extend .bar}', ':not(.foo.bar):not(.foo.x)')
    assert_unification(
      ':not(.foo.bar, .baz.bar)',
      '.x {@extend .bar}',
      ':not(.foo.bar, .baz.bar):not(.foo.x, .baz.x)')
  end
  ...
  def test_extend_into_mergeable_pseudoclasses
    ...
    assert_extends(':nth-column(n of .foo)', '.x {@extend .foo}', ':nth-column(n of .foo, .x)')
    assert_extends(
      ':nth-last-column(n of .foo)',
      '.x {@extend .foo}',
      ':nth-last-column(n of .foo, .x)')
  end
  ...
  def test_complex_extend_into_pseudoclass
    ...
    assert_extends(
      ':nth-column(n of .bar)',
      '.x .y {@extend .bar}',
      ':nth-column(n of .bar, .x .y)')
    assert_extends(
      ':nth-last-column(n of .bar)',
      '.x .y {@extend .bar}',
      ':nth-last-column(n of .bar, .x .y)')
  end

  def test_not_cancels
    # We can safely remove :nots because they don't affect specificity.
    assert_unification(':not(.foo)', ':not(.bar) {@extend .foo}', ':not(.foo), .bar')
    # This shouldn't be unified to "*" because that doesn't preserve specificity.
    assert_unification(':not(.foo)', ':not(.foo) {@extend .foo}', ':not(.foo), .foo')
  end

  def test_not_within_matches
    assert_unification(
      ':matches(.foo, .bar)',
      ':not(.x, .y) {@extend .foo}',
      ':matches(.foo, .bar), :not(.x, .y)')
  end
  ...
  def test_pseudoclasses_merge
    assert_unification(':matches(.foo)', ':matches(.bar) {@extend .foo}', ':matches(.foo, .bar)')
    assert_unification(':-moz-any(.foo)', ':-moz-any(.bar) {@extend .foo}', ':-moz-any(.foo, .bar)')
    assert_unification(':current(.foo)', ':current(.bar) {@extend .foo}', ':current(.foo, .bar)')
    assert_unification(
      ':nth-column(n of .foo)',
      ':nth-column(n of .bar) {@extend .foo}',
      ':nth-column(n of .foo, .bar)')
    assert_unification(
      ':nth-last-column(n of .foo)',
      ':nth-last-column(n of .bar) {@extend .foo}',
      ':nth-last-column(n of .foo, .bar)')
  end
  ...
  def test_matches_unifies_with_matching_value
    assert_unification(".foo", ":matches(.foo) {@extend .foo}", ".foo")
  end

  def test_matches_unifies_with_matches
    assert_equal <<CSS, render(<<SCSS)
:matches(.x, .y, .z) {
  a: b; }
CSS
